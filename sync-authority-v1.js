(() => {
  "use strict";
  if (window.__eggSyncAuthorityV1) return;
  window.__eggSyncAuthorityV1 = true;
  ...
  function applyApp2(remote, forceStartup = false) {
    ...
    } else if (local && localStamp > remoteStamp) {
      return false;
    }
  ...
  async function startListeners() {
    ...
    app2Unsub = f.onSnapshot(
      f.doc(window.FirestoreDB, "entries", APP2_DOC),
      snap => {
        const remote = snap.exists() ? snap.data()?.farmApp2 : null;
  ...
  async function init() {
    ...
    console.log("✅ Firebase source-of-truth authority active");
  }
