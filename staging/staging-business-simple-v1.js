(() => {
  "use strict";
  if (window.__StagingSimpleBusinessV1 || !window.__ChickenEggsStagingMode) return;
  window.__StagingSimpleBusinessV1 = true;
  ...
  const SUITE = "rfpBusinessSuiteV1";
  const IDENTITY = "rfpBusinessIdentityV2";
  ...
  function suiteState() {
    ...
    return {
      ...
      settings: s.settings && typeof s.settings === "object" ? s.settings : {invoicePrefix:"RFP",nextInvoice:1}
    };
  }
  function identity() {
    const x = read(IDENTITY, {});
    return {
      businessName: String(x.businessName || BRAND).replace(/,?\s*LLC\b/ig, "").trim() || BRAND,
  ...
  function notify(key) {
    window.dispatchEvent(new CustomEvent("farm-local-data-changed", {detail:{key,staging:true}}));
    window.dispatchEvent(new CustomEvent("core-data-synced", {detail:{key,staging:true}}));
  }
  ...
  function addExpense(record) {
    ...
    const item = {
      id: String(record?.id || `expense-${Date.now()}`),
    ...
    if (!write(APP2, next)) return false;
    notify(APP2);
    return item;
  }
  ...
  function recentSales() {
    ...
    (Array.isArray(d.entries) ? d.entries : [])
      ...
      .forEach((e,i) => rows.push({key:`egg:${i}`,kind:"egg",date:String(e.date||""),amount:eggSaleAmount(e),raw:e}));
    (Array.isArray(d.business?.chickenSales) ? d.business.chickenSales : [])
      .forEach((s,i) => rows.push({key:`bird:${i}`,kind:"bird",date:String(s?.date||""),amount:chickenSaleAmount(s),raw:s}));
  ...
  function createReceipt(record) {
    const s = suiteState();
    const prefix = String(s.settings.invoicePrefix || "RFP").replace(/[^A-Za-z0-9-]/g, "").slice(0,12) || "RFP";
    const nextNumber = Math.max(1, Math.floor(n(s.settings.nextInvoice) || 1));
    const receipt = {
    ...
    s.settings = {...s.settings, invoicePrefix:prefix, nextInvoice:nextNumber+1};
    s.receipts.push(receipt);
    write(SUITE, s);
    window.dispatchEvent(new CustomEvent("rfp-staging-business-changed", {detail:{simple:true}}));
    return receipt;
  }
  ...
  function ensureUi() {
    ...
    if (!document.getElementById("rfpSimpleBusinessModal")) {
      ...
      m.hidden = true;
      m.innerHTML = `<div class="rfp-simple-sheet"><div class="rfp-simple-head"><div><div class="eyebrow">STAGING • OWNER BUSINESS</div><h2>${BRAND}</h2><div class="rfp-simple-muted">Simple business view — the same expense source used by Home.</div></div><button class="rfp-simple-close" type="button">Close</button></div><div id="rfpSimpleSummary"></div><div class="rfp-simple-tabs"><button type="button" data-simple-tab="expenses">🧾 Expenses</button><button type="button" data-simple-tab="receipts">💵 Sales Receipts</button></div><div id="rfpSimpleBody"></div></div>`;
  ...
  function receiptPanel() {
    ...
    form?.querySelector('[name="sale"]')?.addEventListener("change", e => {
      ...
      if (row.kind === "bird" && row.raw?.buyer) form.elements.customer.value = row.raw.buyer;
    });
    form?.addEventListener("submit", e => {
      e.preventDefault();
      const f = new FormData(e.currentTarget);
      const receipt = createReceipt({customer:f.get("customer"),status:f.get("status"),items:f.get("items"),total:f.get("total")});
  ...
  function renderReceipt(r) {
    ...
    if (!p || !r) return;
    const id = identity();
    p.innerHTML = `<div class="rfp-simple-receipt"><h3>${esc(id.businessName)}</h3>${id.phone || id.email ? `<div>${esc([id.phone,id.email].filter(Boolean).join(" • "))}</div>` : ""}<hr><div><strong>${esc(r.number)}</strong> • ${esc(r.date)}</div><div>Customer: ${esc(r.customer)}</div><div>Items: ${esc(r.items)}</div><div>Status: ${esc(r.status)}</div><h2>Total ${money(r.total)}</h2><small>${esc(id.receiptFooter)}</small></div><div class="rfp-simple-actions" style="margin-top:8px"><button type="button" onclick="window.print()">Print / Save PDF</button></div>`;
  }
  ...
  function init() {
    ensureUi();
    if (!document.getElementById("rfpBusinessLauncher") && retries++ < 20) setTimeout(init, 150);
  }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", () => setTimeout(init, 0), {once:true});
  else setTimeout(init, 0);

  ["farm-local-data-changed","core-data-synced","rfp-staging-business-changed"].forEach(name => window.addEventListener(name, () => {
    if (!document.getElementById("rfpSimpleBusinessModal")?.hidden) renderSummary();
  }));

  window.StagingSimpleBusinessV1 = {
    version:1,
    sources:{entries:ENTRIES,expenses:APP2,chickenSales:BUSINESS},
    calculate,monthStats,ytdStats,addExpense,recentSales,createReceipt,open,close,show,
    firebaseReads:0,firebaseWrites:0,networkCalls:0
  };
  console.log("🧾 STAGING simple Business active — receipts + expenses only; profit/loss uses the same expense source as Home; 0 Firebase calls");
})();
