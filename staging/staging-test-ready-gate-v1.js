(() => {
  ...
  function scriptAlreadyLoaded(name){
    return [...document.scripts].some(s=>String(s.src||"").includes(`/staging/${name}`)||String(s.src||"").includes(name));
  }
  ...
  async function ensureModules(){
    ...
    preparePromise=(async()=>{
      for(const name of MODULES){
        try{await loadScript(name);}catch(error){console.error("STAGING test module load failed:",error);}
      }
      const deadline=Date.now()+15000;
      while(Date.now()<deadline){
        if(suiteReady())return true;
        await sleep(120);
      }
      return false;
  ...
  function lockedReason(){
    if(!dataReady())return "STAGING is still loading its farm runtime.";
    const missing=missingModules();
    return missing.length?`Test setup is missing: ${missing.join(", ")}.`:"The sandbox test is not ready yet.";
  }

  function refresh(){
    const btn=button();if(!btn)return;
    btn.disabled=false;
    btn.dataset.finalSuiteReady=ready()?"true":"false";
    btn.dataset.testLocked=ready()?"false":"true";
  ...
  async function prepareAndRun(btn){
    if(preparing)return;
    preparing=true;refresh();
    try{
      const ok=await ensureModules();
      refresh();
      if(!ok||!ready())throw new Error(lockedReason());
      await window.StagingTestMemoryRunnerV1.run();
    }catch(error){
      console.error("STAGING sandbox preparation failed:",error);
      alert(`Sandbox test could not start.\n\n${String(error?.message||error)}\n\nLIVE data was not changed.`);
