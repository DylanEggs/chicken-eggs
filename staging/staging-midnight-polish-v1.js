(() => {
  ...
  function ensureFarmNote(){
    ...
    let note=document.getElementById("rfpLlcFarmNote");if(!note){note=document.createElement("div");note.id="rfpLlcFarmNote";note.className="rfp-llc-note";const title=farm.querySelector(".screenTitle");if(title)title.insertAdjacentElement("afterend",note);else farm.prepend(note);}
    note.textContent=`Business-facing staging name: ${BRAND} • ${LOCATION}. This preview does not change legal filings or write anything to Firebase.`;
  }
  ...
  window.StagingMidnightPolishV1={version:1,brand:BRAND,location:LOCATION,render,networkCalls:0,writesLocalStorage:false,writesFirebase:false};
  console.log("🌙 STAGING midnight polish active — Rose Family Poultry, LLC branding + local Farm Wins; zero Firebase calls");
})();