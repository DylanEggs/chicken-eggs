(() => {
  ...
  function attachSuite() {
    ...
    window.StagingFullTest = {
      ...
      async run() {
        ...
        return {...first,total:results.length,passed:results.length-failed.length,failed:failed.length,results,suite:`${first?.suite||"staging-full"}+customer-requests-live-parity-v2`};
      },
  ...
  function waitForParity() {
    const data=window.StagingCustomerRequestsLiveParityV1,ui=window.FarmCustomerRequestsV1;
    parityReady=!!data&&!!ui&&String(ui.version||"").includes("staging-parity");
    if(!parityReady){setTimeout(waitForParity,120);return;}
    attachSuite();
    window.dispatchEvent(new CustomEvent("staging-final-suite-changed"));
  }

  window.StagingCustomerRequestStatusTestV1 = {version:5,parityReady:()=>parityReady,runRegression:runParityChecks};
