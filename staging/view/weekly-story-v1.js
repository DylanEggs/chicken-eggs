(() => {
  "use strict";
  if (window.__CustomerWeeklyStoryV1) return;
  window.__CustomerWeeklyStoryV1 = true;
  ...
  function summarize(data = getData()) {
    const daily = Array.isArray(data?.stats?.daily30) ? data.stats.daily30 : [];
    const complete = daily.filter(x => x && x.date).slice(-14);
    ...
    const trend = pct(currentTotal, priorTotal);
    const dozens = currentTotal / 12;
    return {
      days: current.length,
      currentTotal,
      priorTotal,
  ...
  function injectCss() {
    if (document.getElementById("rfpWeeklyStoryCss")) return;
    const s = document.createElement("style");
    s.id = "rfpWeeklyStoryCss";
    s.textContent = `
      ...
      .weekly-story h2{margin:3px 0 6px;font-size:19px;line-height:1.2}.weekly-story p{margin:0;font-size:12px;line-height:1.55;color:#4c6252}.weekly-story strong{color:#17351f}.weekly-story .weekly-story-line{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:10px}.weekly-story .weekly-story-chip{display:inline-flex;align-items:center;gap:5px;padding:7px 9px;border-radius:999px;background:rgba(255,255,255,.78);border:1px solid rgba(31,122,58,.11);font-size:11px;font-weight:800;color:#35543e}
      @media(max-width:560px){.weekly-story{padding:13px 14px}.weekly-story h2{font-size:17px}}
    `;
    document.head.appendChild(s);
  }

  function simplifyCoopReport(data) {
    const grid = document.querySelector("#coopReportTitle")?.closest(".section-block")?.querySelector(".metric-grid");
  ...
  function storyText(s) {
    if (!s.enough) return "The flock is still building enough recent laying history for a weekly story. Check back after a few more collection days.";
    const trendWord = !s.priorEnough ? "" : s.trend > 5 ? ` That's about ${Math.round(Math.abs(s.trend))}% more than the week before.` : s.trend < -5 ? ` That's about ${Math.round(Math.abs(s.trend))}% less than the week before.` : " That's almost exactly in line with the week before.";
    return `Over the last ${s.days} days, the flock laid ${s.currentTotal} eggs — an average of ${s.currentAvg.toFixed(1)} per day.${trendWord}`;
  }
  ...
  function render() {
    ...
    card.innerHTML = `<div class="section-kicker">🌻 This week at ${BRAND}</div><h2>${s.enough ? `${s.currentTotal} eggs from the flock this week` : "The weekly flock story is growing"}</h2><p>${storyText(s)}</p><div class="weekly-story-line"><span class="weekly-story-chip">🥚 ${dozenText}</span><span class="weekly-story-chip">${trendChip}</span></div>`;
    return true;
  }
  ...
  window.CustomerWeeklyStoryV1 = {version:1, brand:BRAND, summarize, storyText, render, networkCalls:0, firebaseReads:0, firebaseWrites:0};
})();