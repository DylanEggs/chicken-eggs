(() => {
  ...
  function blank(){return {version:1,settings:{enabled:true,eggs:"auto",birds:"auto"},requests:[],updatedAt:0};}
  ...
  function settings(){const s=load().settings||{};return {enabled:true,eggs:MSGS[s.eggs]?s.eggs:"auto",birds:MSGS[s.birds]?s.birds:"auto"};}
  ...
  window.StagingCustomerRequestPublicParityV1={version:3,key:KEY,load,save,createRequest,settings,firestoreApi:fs,db:fakeDb,emit};
  ...
  try{
    ...
    console.log("🪞 STAGING customer preview request parity active — request form forced ON with sandbox-only writes");
