(() => {
  ...
  function sanitizeRequest(raw,existing=false){
    ...
    const name=String(r.name||"").trim().slice(0,80),phone=String(r.phone||"").trim().slice(0,40),email=String(r.email||"").trim().slice(0,120);
    ...
    const quantity=Math.min(999,whole(r.quantity)),note=String(r.note||"").trim().slice(0,300),status=STATUS.includes(r.status)?r.status:"New";
    if(!existing){
  ...
  function updateStatus(id,status){
    if(!STATUS.includes(status))throw new Error("Invalid request status.");
    const s=load(),row=s.requests.find(r=>r.id===String(id));
    if(!row)throw new Error("Request not found.");
    row.status=status;row.updatedAt=Date.now();save(s);return row;
  }
  ...
  function ensureScreen(){
    ...
    if(!document.getElementById("customerRequestsCss")){
      const st=document.createElement("style");st.id="customerRequestsCss";st.textContent=`#customerRequests .req-settings{display:grid;grid-template-columns:1fr 1fr;gap:12px}.req-card{padding:14px;border:1px solid rgba(31,122,58,.12);border-radius:18px;background:rgba(255,255,255,.9);margin:10px 0}.req-top{display:flex;justify-content:space-between;gap:12px}.req-contact{font-weight:800;margin-top:5px}.req-meta{color:var(--muted);margin-top:5px}.req-note{margin-top:8px}.req-actions{display:flex;gap:8px;align-items:center;margin-top:10px}.req-actions select{flex:1}.req-status-note{padding:10px 12px;margin:10px 0;border-radius:12px;background:rgba(31,122,58,.12);font-weight:850}@media(max-width:640px){#customerRequests .req-settings{grid-template-columns:1fr}.req-top{display:block}}`;document.head.appendChild(st);
    }
    return screen;
  }

  async function updateStatusFromButton(button){
    ...
    if(!id||!STATUS.includes(status)){statusNotice="❌ Could not read the selected request status.";render();return;}
    button.disabled=true;button.textContent="Updating…";
    try{
      await sleep(60);
      updateStatus(id,status);
      const saved=load().requests.find(r=>r.id===id);
      if(saved?.status!==status)throw new Error(`Status verification failed: expected ${status}, found ${saved?.status||"missing"}`);
      statusNotice=`✅ Request updated to ${status}.`;
      render();
    }catch(error){statusNotice=`❌ ${String(error?.message||error)}`;render();}
  }
  ...
  function render(){
    ...
    body.querySelectorAll("[data-req-save]").forEach(btn=>btn.addEventListener("click",()=>void updateStatusFromButton(btn)));
  }

  function start(){ensureScreen();render();setTimeout(()=>{ensureScreen();render();},700);setTimeout(()=>{ensureScreen();render();},1800);}
  window.StagingCustomerRequestsV1={version:3,key:KEY,load,save,createRequest,updateStatus,updateStatusFromButton,setAvailability,publicAvailability,autoAvailability,sanitizeRequest,render,statuses:STATUS.slice(),birdTypes:{...BIRD_TYPES}};
