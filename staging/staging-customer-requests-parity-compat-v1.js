(() => {
  "use strict";
  if (!window.__ChickenEggsStagingMode || window.StagingCustomerRequestsV1) return;
  const p=window.StagingCustomerRequestsLiveParityV1;
  if(!p||!window.FarmCustomerRequestsV1){console.error("STAGING customer parity compatibility could not start");return;}
  ...
  function setAvailability(kind,value){if(!["eggs","birds"].includes(kind)||!MSGS[value])throw new Error("Invalid availability setting.");const s=p.readSettings();s[kind]=value;p.writeSettings(s);window.FarmCustomerRequestsV1.render();return publicAvailability(kind);}
  function updateStatus(id,status){return p.updateRequest(id,{status,updatedAt:Date.now()});}
  function render(){return window.FarmCustomerRequestsV1.render();}
