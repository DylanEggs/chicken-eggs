(() => {
  ...
  const CLOSED=["Fulfilled","Cancelled"];
  ...
  let fs=null,db=null,unsubscribe=null,rows=[],settings={enabled:false,eggs:"auto",birds:"auto"},busy=false,lastError="",showClosed=false;
  ...
  function render(){
    ...
    const visibleRows=showClosed?rows:openRows;
    const inboxKicker=showClosed?"📨 All requests":"📨 Open request inbox";
    const inboxTitle=showClosed?`${rows.length} total request${rows.length===1?"":"s"}`:`${openRows.length} open request${openRows.length===1?"":"s"}`;
    const emptyText=showClosed?"No customer requests yet.":(rows.length?"No open customer requests.":"No customer requests yet.");
    const closedToggle=closedRows.length?`<button type="button" class="secondary req-closed-toggle" id="reqToggleClosed">${showClosed?"Hide closed":"Show closed"} (${closedRows.length})</button>`:"";
    body.innerHTML=`<div class="farm2-card"><div class="farm2-kicker">📣 Customer request page</div><h3>Availability & request form</h3><label class="req-live-toggle"><input id="reqPublicEnabled" type="checkbox" ${settings.enabled?"checked":""}> Show the request form on the customer page</label><div class="req-settings"><label>Eggs<select id="reqEggMsg">${options(settings.eggs)}</select></label><label>Birds<select id="reqBirdMsg">${options(settings.birds)}</select></label></div><button id="reqSaveSettings">Save Customer Request Settings</button><div class="farm2-subtle" style="margin-top:8px">The form stays hidden until enabled. Customer names, phone numbers, emails and notes are never published back to the customer page.</div>${lastError?`<div class="req-warning" style="margin-top:10px">${esc(lastError)}</div>`:`<div class="req-good" style="margin-top:10px">Private owner connection is active.</div>`}</div><div class="farm2-formRow"><div class="farm2-card"><div class="farm2-kicker">New</div><div class="farm2-moneyBig">${newCount}</div></div><div class="farm2-card"><div class="farm2-kicker">Open requests</div><div class="farm2-moneyBig">${openRows.length}</div></div></div><div class="farm2-card"><div class="farm2-kicker">${inboxKicker}</div><h3>${inboxTitle}</h3>${closedToggle}<div>${visibleRows.length?visibleRows.map(requestCard).join(""):`<div class="farm2-subtle" id="reqOpenInboxEmpty">${emptyText}</div>`}</div></div>`;
    document.getElementById("reqSaveSettings")?.addEventListener("click",saveSettings);
    document.getElementById("reqToggleClosed")?.addEventListener("click",()=>{showClosed=!showClosed;render();});
  ...
  window.FarmCustomerRequestsV1={version:3,openInbox,render,readSettingsForm,rows:()=>rows.slice(),settings:()=>({...settings}),showClosed:()=>showClosed};
