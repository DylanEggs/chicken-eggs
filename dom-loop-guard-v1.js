...
(() => {
  "use strict";
  if(window.__directFlockPhotosV1) return;
  window.__directFlockPhotosV1 = true;
  ...
  function compress(file,cb){if(!file)return cb("");const rd=new FileReader();rd.onload=e=>{const im=new Image();im.onload=()=>{const s=140,side=Math.min(im.width,im.height),sx=(im.width-side)/2,sy=(im.height-side)/2,c=document.createElement("canvas");c.width=s;c.height=s;c.getContext("2d").drawImage(im,sx,sy,side,side,0,0,s,s);cb(c.toDataURL("image/jpeg",.5))};im.onerror=()=>cb("");im.src=e.target.result};rd.onerror=()=>cb("");rd.readAsDataURL(file)}
