...
check('Closed-inbox candidate hides Fulfilled and Cancelled without deleting history',requestParity.includes('closedStatuses=["Fulfilled","Cancelled"]')&&requestParity.includes('card.hidden=!showClosed')&&requestParity.includes('Show closed (')&&requestParity.includes('No open customer requests.'));
check('Closed-request sandbox torture test covers delayed listener, both statuses, persistence and archive reveal',requestStatusTest.includes('setSuppressEmit(true)')&&requestStatusTest.includes('snapshot listener suppressed')&&requestStatusTest.includes('Cancelled request immediately leaves')&&requestStatusTest.includes('Fulfilled request immediately leaves')&&requestStatusTest.includes('Show closed reveals both stored requests'));
