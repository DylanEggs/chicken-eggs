...
for(let i=1;i<=60;i++){const r=await fetch(`${base}staging-build.json?controls=${Date.now()}-${i}`,{cache:'no-store'}).catch(()=>null);if(r?.ok&&String((await r.json()).build||'')===expected)break;if(i===60)throw new Error(`Staging deployment ${expected} not visible for controls E2E`);await sleep(5000);}
...
try{
  ...
  const baseline=await page.evaluate(async()=>{
    ...
    const rows=JSON.parse(beforeEntries||'[]');rows.push({id:'CONTROL-E2E-DESTROY',type:'eggs',date:'2099-12-30',eggs:999});
  ...
  await owner.waitForFunction(()=>window.__ChickenEggsStagingOwnerMode===true&&window.__STAGING_FIREBASE_READONLY__===true,null,{timeout:30000});
  const gate=await owner.evaluate(()=>({title:document.querySelector('#farmOwnerAuthCard h1')?.textContent||'',environment:window.__ChickenEggsEnvironment,ownerMode:window.__ChickenEggsStagingOwnerMode,readOnly:window.__STAGING_FIREBASE_READONLY__,signedIn:window.FarmOwnerAuth?.isSignedIn?.()||false,firestoreExposed:!!window.FirestoreDB,firebaseUserExposed:!!window.FirebaseUser,syncVersion:window.FarmSyncSafety?.version||'',hasPasswordField:!!document.getElementById('farmOwnerPassword'),bannerText:document.getElementById('stagingSafetyBanner')?.innerText||''}));
  ...
  if(gate.signedIn||gate.firestoreExposed||gate.firebaseUserExposed)throw new Error('Owner-gated staging safety boundary failed');
  ...
  console.log(`PASS Staging manual controls + owner-login gate E2E — ${expected}; verified LIVE mirror`);
