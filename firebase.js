...
if (!window.__farmSafeFirebaseV8) {
  ...
  function setStatus(text) {
    try { if (typeof window.setSyncStatus === "function") window.setSyncStatus(text); }
    catch {}
  }
  function setLoading(on) {
    ...
    if (!style) {
      ...
      style.textContent = `.farm-sync-loading .screen button,.farm-sync-loading .bottomNav button{pointer-events:none}.farm-sync-loading #syncStatus{opacity:.85}`;
  ...
  function writeLocal(key, value, detail = {}) {
    ...
    finally { window.__farmApplyingRemote = false; suppressLocalHook = false; }
    window.dispatchEvent(new CustomEvent("farm-data-synced", { detail: { key, authoritative: true, ...detail } }));
  }
  ...
  function mergeDelta(ds, baseValue, localValue, remoteValue) {
    ...
    if (ds.kind === "app2") {
      ...
      const arrays = ["chickenSales"];
      const maps = ["calc"];
      out = applyScalarDelta(base, local, remote, new Set([...arrays, ...maps]));
  ...
  function app2Score(v) {
    ...
    return a.flock.length * 10 + a.expenses.length * 5 + a.customers.length * 4 + a.orders.length * 3 + a.chores.length * 2 + Object.keys(a.saleMeta).length + Math.min(10, a.activity.length) * 0.2;
  }
  ...
  async function safetyCandidates() {
    ...
    try {
      const old = await getDoc(doc(db, "entries", LEGACY_APP2_DOC));
      if (old.exists() && old.data()?.farmApp2) candidates.push({ source: "older Firebase Farm App 2 copy", value: old.data().farmApp2 });
    } catch (e) { console.warn("Legacy recovery copy could not be checked:", e); }
    for (let i = 0; i < SAFETY_SLOTS; i++) {
      try {
        const snap = await getDoc(doc(db, "entries", `farm_safety_app2_${i}`));
        if (snap.exists() && snap.data()?.farmApp2) candidates.push({ source: `Firebase safety backup ${i + 1}`, value: snap.data().farmApp2 });
      } catch {}
    }
    return candidates;
  }
  ...
  async function maybeRecoverApp2(remoteValue) {
    ...
    const rs = app2Score(remote), bs = app2Score(best.value);
    ...
    console.warn(`🛟 Farm recovery candidate selected: ${best.source}. Cloud score ${rs.toFixed(1)} -> recovered score ${app2Score(merged).toFixed(1)}`);
  ...
  async function backupRemote(ds, remoteValue) {
    ...
    } catch (e) { console.warn("Safety backup skipped:", ds.kind, e); }
  }
  ...
  async function pushDataset(ds, options = {}) {
    ...
    const remote = await readRemote(ds);
    const base = baseline.get(ds.key) || remote || {};

    if (!options.recovery && catastrophicDrop(ds, base, local)) {
      console.error(`🛡️ Blocked destructive stale-device write for ${ds.kind}`);
      const restore = remote || base;
      if (restore && typeof restore === "object") writeLocal(ds.key, restore, { protected: true });
    ...
    const payload = options.recovery ? clone(local) : mergeDelta(ds, base, local, remote || base);
    if (remote && !same(remote, payload)) await backupRemote(ds, remote);
  ...
  async function syncDataset(ds) {
    if (!bootstrapComplete) return bootstrapPromise;
  ...
  async function syncAllFarmData() {
    ...
    } catch (e) { console.warn("Safe farm sync error:", e); }
  ...
  function scheduleDatasetSync(ds) {
    ...
    timers.set(ds.key, setTimeout(async () => {
      ...
      catch (e) { console.warn(`${ds.kind} safe sync waiting:`, e); }
    }, 350));
  }
  ...
  async function loadCoreFromCloud() {
    ...
    try {
      ...
      if (bootstrapComplete) setStatus("Offline/local data shown");
  ...
  function startListeners() {
    ...
    onSnapshot(collection(db, "entries"), snap => {
      ...
      for (const change of changes) {
        ...
        if (!remote) continue;
        if (dirty.has(ds.key)) scheduleDatasetSync(ds);
        else {
  ...
  async function bootstrap() {
    ...
    bootstrapPromise = (async () => {
      ...
      while (!auth.currentUser && Date.now() - start < 15000) await new Promise(r => setTimeout(r, 80));
      ...
      if (recovery) {
        ...
        setStatus("Firebase synced " + new Date().toLocaleTimeString());
      }

      lastCloudRefresh = Date.now();
      setLoading(false);
      window.dispatchEvent(new CustomEvent("farm-sync-ready"));
      console.log("✅ Safe Firebase v8 ready — cloud-first startup; stale-device writes blocked");
      return true;
    })().catch(err => {
      console.error("Safe Firebase bootstrap failed:", err);
      setLoading(false);
      setStatus("Offline/local data shown — cloud writes locked");
      throw err;
    });
  ...
  window.refreshCoreFromFirebase = async () => { await safeReady(); return loadCoreFromCloud(); };
  ...
  signInAnonymously(auth).catch(e => {
    ...
    setLoading(false);
    setStatus("Offline/local data shown — Firebase unavailable");
  });

  window.addEventListener("online", async () => {
    try { await safeReady(); await syncAllFarmData(); await loadCoreFromCloud(); }
    catch {}
  });
  document.addEventListener("visibilitychange", () => {
    if (document.hidden || !bootstrapComplete) return;
    if (Date.now() - lastCloudRefresh > 15000) void syncAllFarmData();
  });

  import("./farm-consistency-v2.js?v=6")
    .then(() => import("./app-audit-v1.js?v=2"))
    .then(() => import("./audit-finish-v1.js?v=2"))
    .catch(e => console.warn("Farm audit layer failed to load:", e));
  import("./dom-loop-guard-v3.js?v=2").catch(e => console.warn("Current redraw guard failed to load:", e));
  import("./flock-photo-fix-v2.js?v=5").catch(e => console.warn("Shared flock photo system failed to load:", e));

  console.log("✅ Firebase initialized in protected cloud-first mode");
}
